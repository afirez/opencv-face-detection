...
static bool sortByArea(const vector<Point> &v1, const vector<Point> &v2) {
    double v1Area = fabs(cv::contourArea(Mat(v1)));
    double v2Area = fabs(cv::contourArea(Mat(v2)));
    return v1Area > v2Area;
}

Scanner::Scanner(Mat &bitmapMat) {
    srcBitmapMat = bitmapMat;
}
...
vector<Point> Scanner::scanPoint() {
    ...
    //提取边框
    cv::findContours(scanImage, contours, RETR_EXTERNAL, CHAIN_APPROX_NONE);
    //按面积排序
    ...
    if (contours.size() > 0) {
        ...
        vector<Point> outDP;
        //多变形逼近
        cv::approxPolyDP(Mat(contour), outDP, 0.02 * arc, true);
        //筛选去除相近的点
...
Mat Scanner::resizeImage() {
    ...
    if (maxSize > resizeThreshold) {
        ...
        Mat resizedBitmapMat(size, CV_8UC3);
        cv::resize(srcBitmapMat, resizedBitmapMat, size);
        return resizedBitmapMat;
    }
    return srcBitmapMat;
}
...
Mat Scanner::preprocessImage(Mat &image) {
    ...
    Mat blurMat;
    cv::GaussianBlur(grayMat, blurMat, Size(5, 5), 0);
    Mat cannyMat;
    cv::Canny(blurMat, cannyMat, 0, 5);
    Mat thresholdMat;
    cv::threshold(cannyMat, thresholdMat, 0, 255, CV_THRESH_OTSU);
    return cannyMat;
}
